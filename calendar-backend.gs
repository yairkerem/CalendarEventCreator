...
const EVENT_FIELDS = Object.assign({}, BASE_FIELDS, {
  /* Per event, because one message can revise several — "האימונים של איתי
     עוברים לשש, והמשחק של נועה נדחה לראשון" is two edits to two entries, and
...
function parseText(text, file, history) {
  const system = [
    ...
    '  "האימון של איתי היום בבריכה" = intent="update", findText="אימון איתי",',
    '  location="בריכה", start ריק. הכותב מספר מה השתנה בפעילות שכבר ביומן,',
    ...
    '  להעתיק: שם הפעילות, ואם נאמר שם של אדם אז גם הוא. למשל "אימון איתי".',
